...
class TTSEngine:
    ...
    def run(self) -> Tuple[float, list[int], List[List[float]]]:
        ...
        if settings.config["settings"]["allow_title"]:
            self.call_tts(
                "title", process_text(self.reddit_object["thread_title"]), "title"
            )

        counter = [0, 0]  # first for story second for comment num # TODO add for title

        if settings.config["settings"]["storymode"]:
            if settings.config["settings"]["style"] == 0:
                print_substep("Collecting audio ...", "green")
                self.safe_call_tts(
                    "postaudio", self.reddit_object["thread_post"], "story"
                )
                counter[0] += 1
                ...
                for idx, text in track(
                    ...
                    "dowloading audio.. ",
                    len(self.reddit_object["thread_post"]),
                ):
        ...
        if settings.config["settings"]["allow_comment"]:
            for idx, comment in track(
                ...
                if (
                    settings.config["settings"]["auto_selection"]
    ...
    def safe_call_tts(
        self, fname: str | int, text: str, type: Literal["story", "comment", "title"]
    ) -> None:
        """
    ...
    def split_post(self, text: str, fname, type: str) -> None:
        ...
        split_text = [
            ...
            for x in re.finditer(
                r" *(((.|\n){0,"
                + str(self.tts_module.max_chars - 2)
                + "})(\.|.$))",  # type: ignore idk
                process_text(text),
            )
        ]
        ...
        idy = None
        for idy, text_cut in track(
            enumerate(split_text), "Downloading audio...", total=len(split_text)
        ):
            newtext = text_cut
        ...
        with open(f"{self.path}/list.txt", "w") as f:
            ...
            split_files.append(str(f"{self.path}/{fname}-{idy}.part.mp3"))
    ...
    def call_tts(
        self, filename: Any, text: str, type: str | None, update_time=True
    ) -> None:
...
def process_text(text: str, clean: bool = True) -> str:
    ...
    if lang:
        ...
        assert isinstance(translated_text, str), "tranlator returned wrong type"
