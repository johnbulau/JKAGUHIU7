...
class AWSPolly:
    ...
    def run(self, text, filepath, random_voice: bool = False):
        try:
            session = Session(profile_name="polly-windows-user")
            ...
            try:
                ...
                response = polly.synthesize_speech(
                    Text=self.add_ssml_tag(
                        self.add_speed_tag(
                            text, speed=settings.config["settings"]["tts"]["aws_speed"]
                        )
                    ),
                    OutputFormat="mp3",
                    TextType="ssml",
    ...
    @staticmethod
    def add_ssml_tag(text: str):
        return "<speak>" + text + "</speak>"

    @staticmethod
    def add_speed_tag(text: str, speed=100):
        return f"<prosody rate='{speed}%'>" + text + "</prosody>"
