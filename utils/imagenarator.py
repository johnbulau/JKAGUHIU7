...
def imagemaker(theme, reddit_obj: dict, txtclr) -> None:
    ...
    font = ImageFont.truetype(
        f"fonts/{settings.config['fonts']['normal_font']}",
        settings.config["fonts"]["normal_font_size"],
    )

    if settings.config["settings"]["allow_title"]:
        title = reddit_obj["thread_title"]
        tfont = ImageFont.truetype(
            f"fonts/{settings.config['fonts']['title_font']}",
            settings.config["fonts"]["title_font_size"],
        )
