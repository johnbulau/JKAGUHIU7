...
def male_female_correction(text):
    pattern_mf = r"(\d+)\s*m"
    text = re.sub(pattern_mf, r"\1 M", text)  # covert male female not safe
    # covert male female not safe
    pattern_mf = r"(\d+)\s*f"
    return re.sub(pattern_mf, r"\1 F", text)


def sanitize_text(
    text: str, change_arc=True
) -> str:  # TODO add the chane_acr to places
    ...
    text = text.replace(
        "&#x200b;", ""
    )  # remove the 0 width space https://www.reddit.com/r/OutOfTheLoop/comments/9abjhm/what_does_x200b_mean/
    text = text.replace("&#x200B;", "")
    ...
    if change_arc:
        ...
        for acro, abr in acronyms_table.items():
            result = result.replace(acro, abr)

        result = (
            result.replace("sexual", " intimate")
            .replace("sex", "intercourse")
            .replace("WIBTA", "would i be the idiot")
            .replace("fuck", "duck")
            .replace("dick", "peepee")
            .replace(" ass ", " back ")
        )
    ...
    return male_female_correction(result)
